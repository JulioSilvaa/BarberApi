...
var Customer_Router_exports = {};
__export(Customer_Router_exports, {
  default: () => Customer_Router_default
});
module.exports = __toCommonJS(Customer_Router_exports);
...
var AuthCustomerMiddleware = class {
  auth(req, res, next) {
    ...
    try {
      ...
      const { sub } = import_jsonwebtoken2.default.verify(token, process.env.JWT_ACCESS_SECRET);
      req.customer_id = sub;
      next();
    } catch (error) {
...
router.delete("/:id", AuthCustomer_default.auth, ExpressAdapter.create(Customer_Controller.delete));
router.patch("/:id", AuthCustomer_default.auth, ExpressAdapter.create(Customer_Controller.update));
...
var Customer_Router_default = router;
